...
const Savanna = () => {
  const { statistics, dispatch } = React.useContext(RSLangContext);
  ...
  const searchWord = (correctWord) => {
    for (let i = 0; i < data.length; i++) {
      if (data[i].word === correctWord) {
        return data[i];
      }
    }
    return "not found";
  ...
  const checkAnswer = (w) => {
    if (w === wordToTranslate.translation) {
      ...
      const tempWord = searchWord(wordToTranslate.word);
      dispatch({ type: "INCREMENT_WORDS_LEARNT", payload: tempWord });
      dispatch({
  ...
  const gameOver = () => {
    ...
    falling("gameOver");
    setInGame(false);
    if (score > statistics.savanna.record) {
      dispatch({ type: "SET_NEW_SAVANNA_RECORD", payload: score });
    }
  };
  ...
  const pushWrongWord = () => {
    if (wrongWord.length !== 0 && !wrongWord.includes(wordToTranslate.word)) {
    ...
    <>
      <div className={s.game_wrapper}>
        <div className={s.game_box}>
          <div
            className={`${s.screen} ${s.greeting_bye_screen} ${
              inGame ? `${s.hidden}` : `${s.visible}`
            }`}
          >
            <h3 className={s.title}>Саванна</h3>
            <div className={s.description}>
              Приготовься, как только ты нажмёшь на Старт, игра начнётся. Твоя
              задача быстро переводить падающие слова. Если ты ошибёшься или
              слово упадёт за границы игры, ты потеряешь жизнь! У тебя 7
              жизней... Готов?
            </div>
            <button
              onClick={() => {
          ...
          >
            ...
            <table className={s.stats_table}>
              ...
              <tr>
                ...
                <td>
                  {wrongWord.map((w) => {
                    return <div className={s.wrongWord}>{w}</div>;
                  })}
                  {/* {wrongWord.join(',')} */}
                </td>
              </tr>
          ...
          <div
            className={`${s.screen} ${s.game} ${
              inGame ? `${s.visible}` : `${s.hidden}`
            }`}
            id={s.game}
          >
            <div
