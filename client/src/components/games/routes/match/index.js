...
const Sapper = () => {
  ...
  const searchWord = (word1, word2) => {
    for (let i = 0; i < data.length; i++) {
      if (data[i].word === word1 || data[i].word === word2) {
        return data[i];
      }
    }
    return "not found";
        ...
        Object.values(both).map((w) => {
          if (w === firstWord.word) {
            ...
            setSolvedWords((prevState) => {
              ...
              const tempWord = searchWord(w, e.target.innerText);
  ...
  const gameOver = () => {
    setIsOver((p) => !p);
    setInGame(false);
    if (score > statistics.customGame.record) {
      dispatch({ type: "SET_NEW_MATCH_RECORD", payload: score });
    }
  };
  ...
  const addFirstWord = (e) => {
    if (
      solvedWords.includes(e.target.innerText) ||
      e.target.innerText === firstWord.word ||
      faileddWords.includes(e.target.innerText)
    )
      return;
    ...
    <>
      <div className={s.game_wrapper}>
        <div className={s.game_box}>
          ...
          >
            <h3 className={s.title}>Match</h3>
            <div className={s.description}>Соедини пары слов!</div>
            <button onClick={startGame} className={`blue_button`}>
              Старт
            </button>
          </div>
